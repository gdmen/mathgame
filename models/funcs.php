...
function deleteUsers($users) {
	...
	$i = 0;
  $stmts = array(
    $mysqli->prepare("DELETE FROM ".$db_table_prefix."users WHERE id = ?"),
    $mysqli->prepare("DELETE FROM ".$db_table_prefix."user_permission_matches WHERE user_id = ?"),
    $mysqli->prepare("DELETE FROM settings WHERE user_id = ?"),
    $mysqli->prepare("DELETE FROM game_state WHERE user_id = ?"),
    $mysqli->prepare("DELETE FROM results WHERE user_id = ?"),
    $mysqli->prepare("DELETE FROM rewards WHERE user_id = ?")
  );
	foreach($users as $id){
